/* global __app_id __firebase_config __initial_auth_token */import React, { useState, useEffect, useMemo } from 'react'; import { initializeApp } from 'firebase/app'; import { getAuth, signInAnonymously, signInWithCustomToken, onAuthStateChanged } from 'firebase/auth'; import { getFirestore, doc, collection, query, onSnapshot, updateDoc, deleteDoc, addDoc, serverTimestamp } from 'firebase/firestore';

// ==================================================================================== // --- CONFIGURATION SETUP (ACTION REQUIRED FOR DEPLOYMENT) --- // // When you deploy this app externally (e.g., Netlify/Vercel), you MUST replace // the two placeholder lines below with your actual, private Firebase keys. // DO NOT PASTE THESE KEYS IN THE CHAT FOR SECURITY. // ====================================================================================

// PASTE THIS FINAL BLOCK HERE:

// 1. Define the final configuration object using the secure Netlify variables const firebaseConfig = process.env.REACT_APP_FIREBASE_CONFIG ? JSON.parse(process.env.REACT_APP_FIREBASE_CONFIG) : null;

// 2. Define the variables the rest of your app needs: const appId = process.env.REACT_APP_APP_ID; const initialAuthToken = process.env.REACT_APP_INITIAL_AUTH_TOKEN;

// 3. Initialize Firebase services once const app = initializeApp(firebaseConfig); const auth = getAuth(app); const db = getFirestore(app); // API Configuration const GEMINI_API_URL = https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=;

const MAX_RETRIES = 5; const INITIAL_BACKOFF_MS = 1000;

// Initialize Firebase (outside component for singleton) // Utility functions const formatCurrency = (amount) => { if (isNaN(amount) || amount === null) return 'ETB 0.00'; return ETB ${parseFloat(amount).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}; }; const formatDate = (timestamp) => { if (!timestamp) return 'N/A'; if (timestamp.toDate) { return timestamp.toDate().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', }); } return new Date(timestamp).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', }); }; // --- API CALL FUNCTION with exponential backoff --- const callGeminiApi = async (systemInstruction, userQuery, maxRetries = MAX_RETRIES) => { const payload = { contents: [{ parts: [{ text: userQuery }] }], tools: [{ "google_search": {} }], // Optional: for grounding systemInstruction: { parts: [{ text: systemInstruction }] }, }; for (let i = 0; i < maxRetries; i++) { try { const response = await fetch(GEMINI_API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
...
} }; // --- APP COMPONENT --- const App = () => { const [inventory, setInventory] = useState([]); const [loading, setLoading] = useState(true); const [userId, setUserId] = useState(null); const [isAuthReady, setIsAuthReady] = useState(false); const [isModalOpen, setIsModalOpen] = useState(false); const [isAnalysisModalOpen, setIsAnalysisModalOpen] = useState(false); const [analysisResult, setAnalysisResult] = useState(''); const [isAnalyzing, setIsAnalyzing] = useState(false); const [currentItem, setCurrentItem] = useState(null); const [searchTerm, setSearchTerm] = useState(''); const [filterLowStock, setFilterLowStock] = useState(false);

// 1. Authentication and Initialization useEffect(() => { if (!auth || !db) { console.error("Firebase not initialized."); return; } // Set up Auth State Listener const unsubscribe = onAuthStateChanged(auth, async (user) => { if (user) { setUserId(user.uid); } else { // Sign in anonymously if no user is present try { if (initialAuthToken) { const userCredential = await signInWithCustomToken(auth, initialAuthToken); setUserId(userCredential.user.uid); } else { const userCredential = await signInAnonymously(auth); setUserId(userCredential.user.uid); } } catch (error) { console.error("Auth error:", error); setUserId(crypto.randomUUID()); // Fallback to a random ID if auth fails } } setIsAuthReady(true); });

return () => unsubscribe();
}, []);

// 2. Data Fetching (Firestore onSnapshot) useEffect(() => { if (db && userId) { setLoading(true); // NOTE: This collection path uses the security rule structure for private user data const inventoryRef = collection(db, /artifacts/${appId}/users/${userId}/inventory); const q = query(inventoryRef);
...
const handleSaveItem = async (data) => { if (!userId) { console.error("Error: User not authenticated. Cannot save data."); return; }
...
const handleDeleteItem = async (id) => { if (!userId) return; try { const docRef = doc(db, /artifacts/${appId}/users/${userId}/inventory, id); await deleteDoc(docRef); } catch (e) { console.error("Error deleting document: ", e); } };

// --- AI Analysis Logic ---

const itemDetails = urgentItems.map(item => ({ name: item.name, currentStock: ${item.currentStock} ${item.unit}, reorderLevel: ${item.reorderLevel} ${item.unit}, unitCost: item.unitCost, totalStockValue: item.currentStock * item.unitCost, vendor: item.primaryVendor, vendorContact: item.vendorContact, }));
...
const handleAnalyze = async () => { // ✨ FIX: ADD 'async' HERE! if (!analysisInput.trim()) { alert("Please enter a question or context for analysis."); return; } setIsAnalyzing(true); setAnalysisResult(null);
...
const filteredInventory = useMemo(() => { let list = inventory;
...
const InventoryForm = ({ item, onSave, onClose }) => { const [name, setName] = useState(item?.name || ''); const [stock, setStock] = useState(item?.currentStock || ''); const [reorderLevel, setReorderLevel] = useState(item?.reorderLevel || ''); const [unit, setUnit] = useState(item?.unit || 'kg'); const [cost, setCost] = useState(item?.unitCost || ''); const [vendor, setVendor] = useState(item?.primaryVendor || ''); const [vendorContact, setVendorContact] = useState(item?.vendorContact || ''); const [expectedDelivery, setExpectedDelivery] = useState( item?.expectedDelivery ? formatDateForInput(item.expectedDelivery) : '' );
...
const MarkOrderedModal = ({ item, onMark, onClose }) => { const [deliveryDate, setDeliveryDate] = useState('');
...
const ReorderAnalysisModal = ({ analysis, isAnalyzing, onClose }) => ( <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"> <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-3xl max-h-[80vh] overflow-y-auto"> <h3 className="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">AI Supply Chain Analyst Report</h3> {isAnalyzing ? ( <div className="flex items-center space-x-2 text-blue-600"> <svg className="animate-spin h-5 w-5 mr-3" viewBox="0 0 24 24"> <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle> <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path> </svg> <p>Analyzing inventory and generating plan...</p> </div> ) : ( <p className="text-gray-700 whitespace-pre-line leading-relaxed">{analysis}</p> )} <div className="mt-6 flex justify-end"> <button onClick={onClose} className="px-6 py-2 text-sm font-medium text-white bg-gray-600 rounded-lg hover:bg-gray-700 transition duration-150"> Close Report </button> </div> </div> </div> );

return ( <div className="min-h-screen bg-gray-50 font-sans"> <header className="bg-white shadow-md p-4 sticky top-0 z-10"> <div className="max-w-7xl mx-auto flex justify-between items-center"> <h1 className="text-3xl font-extrabold text-indigo-700">Amen Bar and Restaurant</h1> <button onClick={() => { setCurrentItem(null); setIsModalOpen(true); }} className="px-4 py-2 text-sm font-medium text-white bg-indigo-600 rounded-lg shadow-lg hover:bg-indigo-700 transition duration-150 flex items-center" > <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor"> <path fillRule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clipRule="evenodd" /> </svg> Add New Item </button> </div> </header>
  ...
  <main className="max-w-7xl mx-auto p-4 md:p-8">
    ...
    className={isAnalyzing ? 'px-6 py-3 text-white rounded-full shadow-xl transition duration-150 bg-orange-400 cursor-not-allowed' : 'px-6 py-3 text-white rounded-full shadow-xl transition duration-150 bg-orange-600 hover:bg-orange-700'}
...
// This block replaces the broken <span> element definition (Lines 536-542)

// Note: You must ensure 'statusColor' is defined correctly in your function. <span className={px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColor}}

{status} </span> {item.isOrdered && item.expectedDelivery && ( <p className="text-xs text-blue-500 mt-1"> Delivery: {formatDate(item.expectedDelivery)} </p> )} </td> <td className="px-4 py-3 whitespace-nowrap text-right text-sm text-gray-500">{item.currentStock} {item.unit}</td> <td className="px-4 py-3 whitespace-nowrap text-right text-sm text-gray-500 hidden sm:table-cell">{item.reorderLevel} {item.unit}</td> <td className="px-4 py-3 whitespace-nowrap text-right text-sm text-gray-500 hidden md:table-cell">{formatCurrency(item.unitCost)}</td> <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-semibold text-gray-900">{formatCurrency(totalValue)}</td> <td className="px-4 py-3 whitespace-nowrap text-left text-sm text-gray-500 hidden lg:table-cell">{item.primaryVendor}</td> <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium"> <div className="flex justify-end space-x-1"> <button onClick={() => { setCurrentItem(item); setIsModalOpen(true); }} className="text-indigo-600 hover:text-indigo-900 p-1 rounded-md" title="Edit Item" > <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"> <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zm-5.045 9.172l-1.414 1.414L3 17.071V14.243l6.364-6.364 1.414 1.414-6.364 6.364z" /> </svg> </button> {isLow && !item.isOrdered && ( <button onClick={() => setCurrentItem({ ...item, isMarkingOrdered: true })} className="text-blue-600 hover:text-blue-900 p-1 rounded-md" title="Mark as Ordered" > <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"> <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" /> <path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v3h-3.328a.25.25 0 00-.176.429l1.408 1.407A1 1 0 0114 10.172V14a2 2 0 01-2 2H8a2 2 0 01-2-2v-3.828a1 1 0 01.352-.748l1.408-1.407A.25.25 0 008.328 8H5V5z" clipRule="evenodd" /> </svg> </button> )}
